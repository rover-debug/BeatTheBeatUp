...
public class ParabolaController : MonoBehaviour
{
    ...
    public float midPointHeight = 6.0f;

    public float ReferenceDistance = 20.0f;

    public float ReferenceSpeed = 12f;
    ...
    void Update()
    {
        ...
        else if (Animation && parabolaFly != null && animationTime > parabolaFly.GetDuration())
        {
            ...
            if (Target.name == "Car")
            {
                ...
                Vector3 destination = Target.transform.TransformPoint(Vector3.zero);
                Vector3 ExplosionOrigin = origin + (0.75f * (destination-origin));
                Target.GetComponent<Finale>().Explode(ExplosionOrigin);
            ...
            Destroy(ParabolaRoot);
            //Destroy(this.gameObject);
        }

    }
    private void FormParabolaRoot() 
    {
        ParabolaRoot = new GameObject();

        GameObject point1 = new GameObject();
        GameObject point2 = new GameObject();
        GameObject point3 = new GameObject();

        point1.transform.position = this.gameObject.transform.position;
        point3.transform.position = Target.transform.position;

        Vector3 dest = point3.transform.position;
        Vector3 source = this.gameObject.transform.position;
        dest.y = 0;
        source.y = 0;
        //Debug.Log(Vector3.Distance(source, dest));
        float PercentRef = Vector3.Distance(source, dest) / ReferenceDistance;
        Speed = ReferenceSpeed * PercentRef;
        
        point2.transform.position = new Vector3(point1.transform.position.x + (point3.transform.position.x - point1.transform.position.x) / 2, PercentRef * midPointHeight, point1.transform.position.z +  (point3.transform.position.z - point1.transform.position.z) / 2) ;
        
        point1.transform.parent = ParabolaRoot.transform;
        point2.transform.parent = ParabolaRoot.transform;
        point3.transform.parent = ParabolaRoot.transform;
    }
